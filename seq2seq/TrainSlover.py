        ...
        for epoch in range(self.train_epoch):
            ...
            for batch, data in enumerate(self.train_dataset):
                ...
                with tf.GradientTape() as tape:
                    self.decoder_dict, self.loss_value = self.seq2seq(norm_img, norm_w, label_dense, training)
                    self.decoder_dict = dict(self.decoder_dict)
                    decoder_argmaxs = self.decoder_dict['decoder_argmaxs']
                    s2s_time1 = time_func()
                    logger.info("seq2seq time consume: {}".format(s2s_time1 - s2s_time0))
                    if iter_count % 100 == 0:
                        self.calc_seqacc(label_dense, decoder_argmaxs, True)
    ...
    def test(self):
        ...
        for batch, data in enumerate(self.eval_dataset):
            try:
                ...
                self.decoder_dict, self.loss_value = self.seq2seq(norm_img, norm_w, label_dense, False)
                self.decoder_dict = dict(self.decoder_dict)
                decoder_argmaxs = self.decoder_dict['decoder_argmaxs']
                show_flag = False
                if iter_count % 500 == 0:
                ...
                batch_cnt, batch_cor = self.calc_seqacc(label_dense, decoder_argmaxs, show_flag)
