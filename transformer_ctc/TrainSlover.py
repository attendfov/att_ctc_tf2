    ...
    def train(self, training=True):
        ...
        for epoch in range(self.train_epoch):
            ...
            for batch, data in enumerate(self.train_dataset):
                ...
                att_label_sparse = tf.string_split(att_idx, ',')
                att_label_indices = att_label_sparse.indices
                att_label_values = att_label_sparse.values
                att_label_values = tf.string_to_number(att_label_values, out_type=tf.int32)
                output_shape = [img_path.shape[0], self.max_dec_length]
                att_dense_label = tf.sparse_to_dense(att_label_indices,
                                                     output_shape,
                                                     att_label_values,
                                                     self.charset.get_eosid())

                ctc_label_sparse = tf.string_split(ctc_idx, ',')
                ctc_label_indices = ctc_label_sparse.indices
                ctc_label_values = ctc_label_sparse.values
                ctc_label_values = tf.string_to_number(ctc_label_values, out_type=tf.int32)
                output_shape = [img_path.shape[0], self.vocab_size-1]
                ctc_sparse_label = tf.SparseTensor(indices=ctc_label_indices,
                                                   values=ctc_label_values,
