...
class ResNet18(tf.keras.Model):
  ...
  def call(self, inputs, widths, training=True):
    ...
    ctc_features = features
    att_features = features + self.pos_encoding[:, :features_w, :]

    ## BH*W*D --> B*H*W*D
    ctc_features = tf.reshape(att_features, [features_b, features_h, features_w, -1])
    # BH*W*D --> B*HW*D
    att_features = tf.reshape(att_features, [features_b, features_h * features_w, -1])

    logger.debug("features shape: {}".format(att_features.shape))
    if self.num_layers > 0:
        mask = None
        ...
        att_features = self.dropout(att_features, training=training)
        for i in range(self.num_layers):
            att_features = self.transfer_enc_layers[i](att_features, training, mask)

    return att_features, weight_mask, ctc_features, length_mask
...
if __name__ == '__main__':
    ...
    ecnoder = ResNet18(1)
    att_features, weight_mask, ctc_features, ctc_length = ecnoder(data, widths, True)
    print("att_features shape: {}".format(att_features.shape))
    print("att_weight_mask shape: {}".format(weight_mask.shape))

    print("ctc_features shape: {}".format(ctc_features.shape))
    print("ctc_length shape: {}".format(ctc_length.shape), ctc_length)
