...
class Seq2Seq(tf.keras.Model):
    ...
    def call(self, input, input_widths, target_input, target_length, training):
        # (batch_size, inp_seq_len, d_model), (batch_size, inp_seq_len)
        enc_output, weight_mask = self.encoder(input, input_widths, training)
        ...
        if target_length is None:
            ...
            target_length = self.get_target_length(target_input, self.eos_id)
            target_mask = tf.sequence_mask(target_length, steps, dtype=tf.float32)
        else:
            target_mask = tf.sequence_mask(target_length, steps, dtype=tf.float32)
        ...
        dec_output, attention_weights = self.decoder(target_input, enc_output, training, look_ahead_mask, dec_padding_mask)
        final_output = self.final_layer(dec_output)  # (batch_size, tar_seq_len, target_vocab_size)

        loss_value = 0.0
        if training:
            loss_value = self.loss(target_input, final_output, target_length)
        return final_output, attention_weights, loss_value

    def loss(self, targets_inputs, inference_inputs, targets_length=None):
        batch = int(targets_inputs.shape[0])
        steps = int(targets_inputs.shape[1])
        batch_eos = tf.expand_dims([self.eos_id] * batch, axis=1)
        targets_inputs = tf.slice(targets_inputs, [0, 1], [batch, steps-1])
        targets_inputs = tf.concat([targets_inputs, batch_eos], axis=1)

        if targets_length is None:
            targets_length = self.get_target_length(targets_inputs, self.eos_id)

        input_mask = tf.cast(tf.sequence_mask(targets_length, steps), dtype=tf.float32)
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=targets_inputs,
                                                              logits=inference_inputs) * input_mask
    ...
    def evaluate(self, input_image, input_widths=None, max_length=40):
        training = False
        ...
        enc_output, weight_mask = self.encoder(input_image, input_widths, training)
